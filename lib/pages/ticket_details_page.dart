...
//  • Escalation permissions: Accept button controlled by isAccept flag
//    from enterprise_escalation_user_rule.json_data.
//  • Reassign button now controlled by reassign flag from same json_data.
//  • _loadEscalationPermissions() fetches the user's escalation
...
class _TicketDetailPageState extends State<TicketDetailPage> {
  ...
  // Separate flag for Accept/Close button actions — does NOT trigger the
  // full-screen overlay. Only shows spinner inside the button itself.
  ...
  void initState() {
    ...
    _webSocketSubscription = WebSocketService().stream.listen((data) {
      ...
      final payload = (data['data'] is Map)
          ...
          : data;
      final type = (data['type'] ?? payload['type'] ?? '').toString().toUpperCase();
      final taskId = (payload['service_request_id'] ??
  ...
  Future<void> _updateStatus(String newStatus) async {
    ...
    if (isAccept) {
      unawaited(NotificationService.instance.stopAlert(
        serviceRequestId: _serviceRequestId.toString(),
        orderId: _serviceRequestId.toString(),
      ));
    }
    ...
    try {
      result = isAccept
          ? await TaskService().acceptServiceRequest(
      ...
      if (mounted) {
        ...
        AppSnackBar.show(context, 'Unable to update request. Please try again.',
            isError: true);
      }
      return;
    }
    ...
    // Apply the confirmed status before any reconciliation network work. This
    // immediately updates the chip and removes Accept/Close buttons.
    //
    // ORDER: callback first (HomePage filter switches before pop), then toast,
    // then pop. Toast fires on TDP context which is still mounted at this point.
    // After pop it appears over the HomePage — user sees correct filter + confirmation.
    if (isClose) {
      widget.onClose?.call();
      if (mounted) {
  ...
  Future<void> _addNote() async {
    ...
    // Get current user name to ensure it's displayed immediately
    final currentUserName = await UserSessionHelper.getUserName() ?? '';
    ...
    setState(() {
      ...
      // Create note object with user name if not already present
      final noteToAdd = returnedNote.isEmpty
          ? {
              'note_text': savedNote,
          ...
          : {
              ...returnedNote,
              // Ensure user_name is present, use current user if missing
              'user_name': returnedNote['user_name']?.toString().isNotEmpty == true
                  ? returnedNote['user_name']
                  : currentUserName,
            };
  ...
  Widget _buildInfoCard() {
    ...
    final rawAssigned    = raw['accepted_by_user_name'] ?? _task['accepted_by_user_name'] ?? raw['assigned_to_name'] ?? _task['assigned_to_name'] ?? _task['assignedTo'] ?? raw['recent_reassigned_to_user_name'];
  ...
  // ── Notes history card ───────────────────────────────────────────────────
  //
  // Decodes task['notes'] — either a List or a JSON-encoded string from
  // JSON_ARRAYAGG — and renders every note with author initials, name, time,
  // and body text. If no notes exist, shows a gentle empty-state row.

  List<Map<String, dynamic>> _parseNotesList() {
    final raw = _task['notes'];
    List decoded = const [];
    ...
    // Ensure every item is a proper Map<String, dynamic>
    return decoded
        .whereType<Map>()
        .map((e) => Map<String, dynamic>.from(e))
        .where((e) {
      final text = (e['note_text'] ?? '').toString().trim();
      return text.isNotEmpty && text != 'null';
    }).toList();
  }

  Widget _buildNotesCard() {
    final notes = _parseNotesList();
        ...
        children: [
          ...
          if (notes.isEmpty)
            ...
            ListView.separated(
              ...
              itemBuilder: (_, i) {
                final note     = notes[i];
                final noteText = (note['note_text'] ?? '').toString().trim();
                final userName = (note['user_name'] ?? '').toString().trim();
                final createdAt =
                    (note['created_at'] ?? '').toString().trim();

                // Build initials (up to 2 chars) for the avatar
  ...
  Widget _buildActions() {
    ...
    // ── While an action is in flight, hide ALL buttons immediately. ──────────
    // This prevents double-taps and gives instant visual feedback that the tap
    // registered — the button disappears, spinner on it was the last thing seen,
    // then the page pops to the correct tab after the API resolves.
    // ────────────────────────────────────────────────────────────────────────
    if (_isActionLoading) return const SizedBox.shrink();
    ...
    return Column(children: [
      if (showAccept) ...[
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: _isActionLoading ? null : () => _updateStatus('IN_PROGRESS'),
      ...
      Row(children: [
        ...
        if (showClose)
          Expanded(
            child: ElevatedButton.icon(
              onPressed: _isActionLoading ? null : _closeTask,
              icon: _isActionLoading
                  ? const SizedBox(
                      width: 14,
                      height: 14,
