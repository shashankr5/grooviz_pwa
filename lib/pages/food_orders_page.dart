...
class FoodOrdersPageState extends State<FoodOrdersPage>
  ...
  String? _readyingOrderNo;
  String? _cancellingOrderNo;
  ...
  // Debounce: collapses burst WS events (a single accept fires 3–5 events)
  // into one silent background sync. Only used as Tier 2 fallback when the
  // WS payload alone isn’t enough to fully resolve the order locally.
  ...
  Future<void> _initializeWebSocket() async {
    ...
    _wsSubscription = ws.stream.listen((event) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        // Always unbox nested data payloads — some Lambdas wrap fields in 'data'.
        final payload = (event['data'] is Map)
            ? Map<String, dynamic>.from(event['data'] as Map)
            : event;
        switch (type) {
          case 'NEW_FOOD_ORDER':
            _handleSocketNewOrder(payload);
            break;
          case 'ORDER_ACCEPTED':
          ...
          case 'FOOD_ORDER_CANCELLED':
          case 'FOOD_ORDER_STATUS':
          case 'SERVICE_ORDER':
            _handleSocketUpdate(payload);
            break;
          case 'ORDER_ETA_UPDATED':
            _handleSocketEtaUpdate(payload);
            break;
          case 'RUSH_HOUR_UPDATED':
            _handleSocketRushHourUpdate(payload);
            break;
          default:
            // Unknown type — do nothing. Silent sync is only for known categories.
            break;
        }
      });
    });
  ...
  void _handleSocketNewOrder(dynamic data) {
    ...
    try {
      ...
      // Payload couldn’t be parsed into a full order — silent sync (Tier 2).
      _syncDebouncer(() {
        if (mounted) _loadFoodOrders(silent: true);
      });
    }
  }

  void _handleSocketUpdate(dynamic data) {
    final orderNo = (data['order_number'] ??
            data['orderNo'] ??
            data['order_id'] ??
            data['summary_id'] ??
    ...
    if (orderNo == null || orderNo.isEmpty) {
      // Order identifier missing from payload — trigger debounced silent sync
      _syncDebouncer(() {
        if (mounted) _loadFoodOrders(silent: true);
      });
      return;
    ...
    if (newStatus == 'DELIVERED') {
      ...
      if (index != -1) {
        // Tier 1: in-place move. WS payload is complete — skip sync.
        setState(() {
        ...
        // Order not in list — may be new. Use silent sync (Tier 2).
        _syncDebouncer(() {
          if (mounted) _loadFoodOrders(silent: true);
        });
      }
    ...
    if (newStatus == 'ACCEPTED') {
      if (index != -1) {
        // Tier 1: patch status + provide a provisional timestamp
        setState(() {
      ...
      // Always trigger debounced silent sync to get accurate server details (e.g. accepted_by user info)
      _syncDebouncer(() {
        if (mounted) _loadFoodOrders(silent: true);
      });
      return;
    ...
    if (index == -1) {
      // Unknown order — silent sync (Tier 2).
      _syncDebouncer(() {
        if (mounted) _loadFoodOrders(silent: true);
      });
      return;
  ...
  // ====================== DATA LOADING ======================
  //
  // [silent] — when true (WS Tier 2 path), _isLoading is never set to true.
  // The full order lists are still replaced from server data, but the user
  // never sees a spinner. Cold loads and pull-to-refresh use silent:false (default).
  ...
  void initState() {
    ...
    // Reload all food orders at midnight so the list reflects the new day.
  ...
  Future<void> _markReady(Map<String, dynamic> order) async {
    ...
    final orderNo = order['orderNo']?.toString() ?? '';
    if (_actionLoadingOrderNo != null || _readyingOrderNo != null) return;
    setState(() {
      _actionLoadingOrderNo = orderNo;
      _readyingOrderNo = orderNo;
    });
    final summaryId = _resolveSummaryId(order);
    if (summaryId == null) {
      _showError("Order ID missing. Please refresh.");
      setState(() {
        _actionLoadingOrderNo = null;
        _readyingOrderNo = null;
      });
      return;
    }
    final Map<String, dynamic> result;
    try {
      result = await _foodOrderService.updateFoodOrderStatus(
          summaryId: summaryId,
      ...
      if (mounted) {
        ...
        _showError('Unable to update order. Please try again.');
      }
      return;
    }
    if (!mounted) return;
    if (result["success"] != true) {
      ...
      if (mounted) {
        setState(() {
          ...
          selectedFilter = FoodOrderStatus.ready.label;
          _actionLoadingOrderNo = null;
          _readyingOrderNo = null;
        });
        AppSnackBar.show(context, "Order marked Ready ✅");
        _loadFoodOrders(silent: true); // fire-and-forget — no await
      }
    }
  }
            ...
            child: Column(
              ...
              children: [
                ...
                ...items.map((i) {
                  ...
                  final qty = i['qty'] ?? 1;
                  final double price = (i['price'] as num?)?.toDouble() ?? 0.0;
                  final double totalPrice = (i['totalPrice'] as num?)?.toDouble() ?? (price * (qty is num ? (qty as num).toDouble() : 1.0));
                  final double displayPrice = totalPrice > 0 ? totalPrice : (price > 0 ? price : 0.0);
                  final String priceStr = displayPrice > 0
                      ? '₹${displayPrice.toStringAsFixed(displayPrice.truncateToDouble() == displayPrice ? 0 : 2)}'
                      : '';
                  ...
                  return Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        _fssaiIcon(_isVeg(i['isVeg'], name)),
                        const SizedBox(width: 7),
                        Expanded(
                          child: Text(name,
                              style: const TextStyle(
                ...
                // Total amount
                Builder(builder: (_) {
                  double total = (order['totalOrderPrice'] as num?)?.toDouble() ?? 0.0;
                  if (items.isNotEmpty) {
                    final itemsSum = items.fold<double>(0.0, (sum, i) {
                      final p = (i['price'] as num?)?.toDouble() ?? 0.0;
                      final tp = (i['totalPrice'] as num?)?.toDouble();
                      final q = (i['qty'] as num?)?.toDouble() ?? 1.0;
                      return sum + (tp != null && tp > 0 ? tp : (p * q));
                    });
                    if (itemsSum > 0 && (total <= 0 || items.length > 1 || total < itemsSum)) {
                      total = itemsSum;
                    }
                  }
                    ...
                    child: Row(
                      ...
                      children: [
                        const Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.receipt_long_rounded, size: 13, color: Colors.black),
                            SizedBox(width: 6),
                            Text('Total Amount',
                                style: TextStyle(
                                    fontSize: 12,
                    ...
                    children: [
                      Expanded(
                        child: _actionButton(
                          ...
                          color: AppColors.success,
                            isLoading: _actionLoadingOrderNo == orderNoStr &&
                              _readyingOrderNo != orderNoStr,
                          onTap: () async {
                            if (_actionLoadingOrderNo != null) return;
                            final rs = _rawStatus(order);
                            if (rs == "DELIVERED" || rs == "CANCELLED") {
                              _showError(null);
                              await _loadFoodOrders();
                            ...
                            unawaited(NotificationService.instance.stopAlert(
                              foodOrderSummaryId: summaryId.toString(),
                              orderId: summaryId.toString(),
                            ));

                            // ── UI FEEL REQUIREMENT: LOADER FIRST, THEN NAVIGATE ───────────────
                            // Show loading state IMMEDIATELY before any API call or UI change.
                            // This prevents the jarring visual of the count changing before
                            // navigation completes. User sees: tap → loader → navigate → done.
                            // This feels more intentional and polished than instant count change.
                            // ────────────────────────────────────────────────────────────────────
                            setState(() {
                              _acceptingIndex = index;
                              _actionLoadingOrderNo = orderNoStr;
                            });
                            ...
                            if (result["success"] != true) {
                              // API failed - clear loader and show error
                              setState(() {
                              ...
                              // Mutate in-place — same pattern _handleSocketUpdate uses for ACCEPTED events.
                              // _filteredOrders getter picks this up immediately via order['status'] check.
                              //
                              // RACE NOTE: if WebSocket delivers ACCEPTED before background reload finishes,
                              // _handleSocketUpdate fires _loadFoodOrders() again. Two concurrent reloads is
                              // harmless — _loadFoodOrders is idempotent. No action needed.
                              //
                              // ETA NOTE: server returns etaTime in acceptFoodOrder response. Background
                              // reload picks it up. WebSocket ACCEPTED handler also picks it up via
                              // _handleSocketUpdate → _loadFoodOrders. Either path is fine.
                              setState(() {
                                order['status'] = FoodOrderStatus.preparing.label;
                                order['raw']['order_status'] = 'ACCEPTED';
                                selectedFilter = FoodOrderStatus.preparing.label;
                                _acceptingIndex = null;
                                _actionLoadingOrderNo = null;
                              });
                      ...
                      Expanded(
                        child: _actionButton(
                          ...
                          color: AppColors.error,
                          isLoading: _cancellingOrderNo == orderNoStr,
                          ...
                          builder: (context, _) {
                            ...
                            return Opacity(
                              ...
                              child: _actionButton(
                                ...
                                isLoading: _readyingOrderNo == orderNoStr,
          ...
          child: Column(
            ...
            children: [
              ...
              ...cancelReasons.map((reason) => InkWell(
                onTap: () async {
                  if (isCancelling || _actionLoadingOrderNo != null) return;
                  final summaryId = _resolveSummaryId(order);
                  if (summaryId == null) {
                    Navigator.pop(context);
                    _showError("Order ID missing. Please refresh.");
                  ...
                  if (mounted) {
                    setState(() {
                      _actionLoadingOrderNo = order['orderNo']?.toString();
                      _cancellingOrderNo = order['orderNo']?.toString();
                    });
                  }
                  if (order["raw"] == null) {
                    if (mounted) {
                  ...
                  // At this point the order is already correctly staged:
                  //   - order['status'] = cancelled (optimistic, done above)
                  //   - order['cancelReason'] = reason (done above)
                  //   - foodOrders.remove(order) done above
                  //   - cancelledOrders.insert(0, order) done above
                  //
                  // So switching to Cancelled filter immediately is safe — the order is already
                  // in cancelledOrders and _filteredOrders will return it instantly.
                  //
                  // MOUNTED GUARD: Navigator.pop() is called first, which triggers a frame.
                  // The if (mounted) below guards the setState and snackbar. The existing
                  // if (!mounted) return earlier in this onTap already guards the API result
                  // handling. Both guards are in place — no gap.
                  Navigator.pop(context);
                  if (mounted) {
                    setState(() => selectedFilter = FoodOrderStatus.cancelled.label);
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      if (isCancelling && _actionLoadingOrderNo == order['orderNo']?.toString())
                        const SizedBox(
                          width: 16,
                          height: 16,
