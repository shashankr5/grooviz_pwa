...
// Redesign: two-line AppBar, SkeletonCheckoutCard loading, AppColors tokens,
// tailored empty states, overflow-safe layouts.
...
class _GuestCheckoutHistoryPageState
  ...
  String _stayDuration(String? checkIn, String? checkOut) {
    final i = _parseDate(checkIn); final o = _parseDate(checkOut);
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (ctx, snap) {
          ...
          final all = snap.data ?? [];
          final filtered = _search.isEmpty ? all : all.where((g) {
          ...
          return Column(children: [
            ...
            if (all.isNotEmpty)
              Container(width: double.infinity, color: AppColors.bg,
...
class _HistoryCard extends StatelessWidget {
  ...
  const _HistoryCard({required this.guest, required this.expanded,
      required this.billFuture, required this.onTap,
      required this.formatDateTime, required this.formatDateOnly,
        ...
        child: Column(children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const Text('Checked Out', style: TextStyle(fontSize: 11,
                        fontWeight: FontWeight.w600,
          ...
          if (expanded) ...[
            ...
            Padding(padding: const EdgeInsets.all(16),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start, children: [
                // Stay grid — 2-column using Flexible
                Row(children: [
                  Expanded(child: _MiniDetail(
                      icon: Icons.login_rounded, label: 'Check-in',
                      value: formatDateOnly(
                          raw['checked_in_time']?.toString()))),
                  const SizedBox(width: 10),
                  Expanded(child: _MiniDetail(
                      icon: Icons.logout_rounded, label: 'Checked Out',
                ...
                if (guest['contact']?.toString().isNotEmpty == true) ...[
                  const SizedBox(height: 10),
                  _MiniDetail(icon: Icons.phone_outlined,
                      label: 'Contact', value: guest['contact']),
                ],
                if (guest['email']?.toString().isNotEmpty == true) ...[
                  const SizedBox(height: 10),
                  _MiniDetail(icon: Icons.email_outlined,
                      label: 'Email', value: guest['email']),
                ],
                if ((raw['device_name'] ?? '').toString().isNotEmpty) ...[
                  const SizedBox(height: 10),
                  _MiniDetail(icon: Icons.devices_outlined,
                      label: 'Devices',
