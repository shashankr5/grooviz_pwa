...
class OrderHistoryDetailSheet extends StatelessWidget {
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...ticket.items.map((item) {
                    final displayPrice = item.totalPrice > 0 ? item.totalPrice : (item.price > 0 ? item.price * item.quantity : 0.0);
                    final priceStr = displayPrice > 0
                      ...
                      child: Row(
                        ...
                        children: [
                          const Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(Icons.receipt_long_rounded, size: 14, color: Colors.black),
                              SizedBox(width: 6),
                              Text('Total Amount',
                                  style: TextStyle(
                                    fontSize: 13,
                          ...
                          Text(
                            '\u20b9${ticket.totalOrderPrice.toStringAsFixed(ticket.totalOrderPrice.truncateToDouble() == ticket.totalOrderPrice ? 0 : 2)}',
                            style: const TextStyle(
                              fontSize: 15,
  ...
  bool _isOrderEscalated(Map<String, dynamic> order) {
    ...
    if (rawDynamic is Map &&
        (rawDynamic['is_escalated'] == 1 || rawDynamic['is_escalated'] == true)) {
      return true;
    }
    return false;
  }
