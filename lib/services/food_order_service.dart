...
class FoodOrderService {
  ...
  Future<Map<String, dynamic>> acceptFoodOrder({
    ...
    // Immediately stop ringing sound/alert for this food order
  ...
  static List<Map<String, dynamic>> _mapFoodOrders(List raw) {
    return raw.map<Map<String, dynamic>>((o) {
      ...
      final statusVal    = m["order_status"] ?? m["status"] ?? "Pending";
      final orderTimeVal = m["created_at"] ?? m["order_time"];
        final escalationHistory = _parseEscalationHistory(m["escalation_history"])
          .where((entry) => (entry["level"] as num?)?.toInt() != null &&
            (entry["level"] as num).toInt() > 0)
          .toList();
      ...
      return {
        ...
        "isVeg":               m["is_veg"],
        "price":               m["food_price"] ?? m["item_price"] ?? m["price"] ?? m["food_item_price"] ?? m["rate"] ?? 0,
        "totalPrice":          m["total_price"] ?? m["item_total"] ?? m["item_total_price"] ?? m["total_amount"] ?? 0,
        // ETA fields (new v1)
        ...
        // Escalation fields
        "isEscalated":         (m["is_escalated"] == 1 || m["is_escalated"] == true) && escalationHistory.isNotEmpty,
