...
class BluetoothPrinterService extends PrinterService {
  ...
  Future<void> print(PrintRequest request) async {
    ...
    try {
      final ticket = await _buildTicket(request.ticket, isReprint: request.isReprint);
      await _manager.printTicket(ticket);
      _setStatus(PrinterStatus.success);
      dev.log('[BTPrinter] printed KOT ${request.ticket.orderNumber} (reprint: ${request.isReprint})');
    } on PrinterException {
  ...
  Future<void> enqueueAndPrint(KOTTicketModel ticket, {bool isReprint = false}) async {
    final request = PrintRequest(ticket: ticket, createdAt: DateTime.now(), isReprint: isReprint);
    _queue.add(request);
    await _saveQueue();
    dev.log('[BTPrinter] enqueued ${ticket.orderNumber} (queue: ${_queue.length}, reprint: $isReprint)');
  ...
  Future<void> reprint(KOTTicketModel ticket) async {
    ...
    await enqueueAndPrint(ticket, isReprint: true);
  }
  ...
  Future<uep.Ticket> _buildTicket(KOTTicketModel kot, {bool isReprint = false}) async {
    ...
    // Smaller KOT header
    ticket.text('KOT', style: const uep.PrintTextStyle(bold: true), align: uep.PrintAlign.center);
    ticket.text(headerName, align: uep.PrintAlign.center);
    ticket.separator();

    // Compact order info
    ticket.text('Room: ${kot.roomNumber} | Order: ${kot.orderNumber}', style: const uep.PrintTextStyle(bold: true));
    if (kot.guestName != null && kot.guestName!.trim().isNotEmpty) {
      ticket.text('Guest: ${kot.guestName}');
    }
    ticket.text('${DateFormatter.formatDateTime(kot.orderTime.toIso8601String().replaceAll('T', ' '))}');
    ticket.separator();
    ...
    for (final item in kot.items) {
      ...
      ticket.text('${item.quantity}x ${item.name} (${item.isVeg ? 'VEG' : 'NON-VEG'})$priceStr');
      if (item.instructions != null && item.instructions!.trim().isNotEmpty) {
        ticket.text('  * ${item.instructions!.trim()}');
      }
    }

    if (kot.totalOrderPrice > 0) {
      ticket.separator();
      final totalFormatted = kot.totalOrderPrice.toStringAsFixed(kot.totalOrderPrice.truncateToDouble() == kot.totalOrderPrice ? 0 : 2);
      ticket.text('TOTAL AMOUNT: Rs.$totalFormatted', style: const uep.PrintTextStyle(bold: true));
    }

    if (kot.cookingInstructions != null &&
        kot.cookingInstructions!.trim().isNotEmpty) {
      ticket.separator();
      ticket.text('INSTRUCTIONS:', style: const uep.PrintTextStyle(bold: true));
      ticket.text(kot.cookingInstructions!.trim());
    }

    ticket.separator();
    // Show different copy labels based on whether it's a reprint
    final copyLabel = isReprint ? '*** GUEST COPY ***' : '*** KITCHEN COPY ***';
    ticket.text(copyLabel, style: const uep.PrintTextStyle(bold: true), align: uep.PrintAlign.center);
    ticket.feed(2);
    ticket.cut();
    return ticket;
  }
  ...
  Future<void> _saveQueue() async {
    try {
      ...
      final json = _queue.map((r) => {
        ...
        'totalOrderPrice':     r.ticket.totalOrderPrice,
      ...
      _queue
        ...
        ..addAll(list.map((m) {
          ...
          return PrintRequest(
            ...
            isReprint:  (map['isReprint'] as bool? ?? false),
          );
        }));
  ...
  Future<void> _saveHistory() async {
    try {
      ...
      final json  = _history.map((t) => {
        ...
        'totalOrderPrice':     t.totalOrderPrice,
        'items':               t.items.map((i) => {
          'name':         i.name,
          'quantity':     i.quantity,
          'isVeg':        i.isVeg,
  ...
  KOTTicketModel _ticketFromMap(Map<String, dynamic> m) {
    return KOTTicketModel(
      ...
      totalOrderPrice:     (m['totalOrderPrice'] as num?)?.toDouble() ?? 0.0,
      items: (m['items'] as List? ?? []).map<KOTItem>((i) {
        ...
        return KOTItem(
          ...
          totalPrice:   (im['totalPrice'] as num?)?.toDouble() ?? 0.0,
        );
      }).toList(),
    );
  }
