class DateFormatter {
  ...
  /// Expects "YYYY-MM-DD HH:MM:SS", "YYYY-MM-DD", or ISO-8601 "YYYY-MM-DDTHH:MM:SSZ"
  /// and returns "DD/MM/YY".
  static String formatDateOnly(String? s) {
    if (s == null || s.trim().isEmpty) return '—';
    try {
      // Normalise: replace T separator so DateTime.parse handles both formats
      String norm = s.trim();
      if (norm.contains('T') && !norm.contains(' ')) {
        // ISO-8601 — parse as-is (DateTime.parse handles Z suffix fine)
      } else if (norm.contains(' ') && !norm.contains('T')) {
        norm = norm.replaceFirst(' ', 'T');
      }
      ...
      // Fallback: best-effort split on '-'
      try {
        final datePart = s.trim().split(RegExp(r'[\sT]')).first;
